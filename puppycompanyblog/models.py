...
class BlogPost(db.Model):
    ...
    def __init__(self, title, text, weight, date, user_id):
        self.date = date
        self.weight = weight
        self.title = title
        self.text = text
...
class Recipe(db.Model):
    ...
    #recipe_image = db.Column(db.String(20), nullable=False, default = 'default_recipe.png')

    user = db.relationship(User)
    user_id = db.Column(db.Integer,db.ForeignKey('users.id'),nullable=False)
        ######### 
    def __init__(self, title, ingredients, user_id, method ):
        self.title = title
